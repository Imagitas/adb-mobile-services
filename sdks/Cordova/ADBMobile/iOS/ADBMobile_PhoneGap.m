...
- (void)pluginInitialize
{
	...
	NSString* pathFromWWW = [NSString stringWithFormat:@"%@/%@", basePath, @"res/js/ADBMobileConfig.json"];

	NSLog ( @"ADBMobile - overriding config path:  %@", pathFromWWW );
	// if a configPath was not passed in, return
	if (pathFromWWW == (id)[NSNull null]) {
		return;
	}
	if (pathFromWWW != nil) {
		[ADBMobile overrideConfigPath:pathFromWWW];
		pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
	} else {
		pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Config Path was null"];
	}
