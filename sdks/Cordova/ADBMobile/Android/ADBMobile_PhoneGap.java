...
public class ADBMobile_PhoneGap extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("getVersion")) {
            ...
            return true;
        } else if (action.equals("overrideConfigPath")) {
            this.overrideConfigPath(args, callbackContext);
            return true;
        }
    ...
    private void overrideConfigPath(JSONArray args, final CallbackContext callbackContext) throws JSONException{
        final String configPath = args.getString(0);

        cordova.getThreadPool().execute((new Runnable() {
            @Override
            public void run() {
                try {
                    InputStream configInput = cordova.getActivity().getApplicationContext()
                            .getAssets().open(configPath);
                    Config.overrideConfigStream(configInput);
                    callbackContext.success("Set Config Path to: " + configPath);
                } catch (IOException e) {
                    e.printStackTrace();
                    callbackContext.error(e.getMessage());
                }
